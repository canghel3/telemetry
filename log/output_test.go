...
func BenchmarkOutputToFile(b *testing.B) {
	for i := 0; i < b.N; i++ {
		//58k characters word below drops performance to 758238 ns/op
		...
		//6543 ns/op
		Stdout().Info().Log("trying to type a relatively long message that is representative for a vast majority of logs")
	}
}
